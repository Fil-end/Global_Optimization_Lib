...
def lasp_ssw_calc(atom, type):
    write_arc([atom])
    if type == 's' or 'short':
        atom.calc = LASP(task='short-global-opt', pot='PdO', potential='NN D3')
    elif type == 'l' or 'long':
        atom.calc = LASP(task='long-global-opt', pot='PdO', potential='NN D3')
...
if __name__ == '__main__':
    ...
    env = MCTEnv(save_dir = 'save_dir', save_every= 20 ,timesteps = 1000, reaction_H = 0.790, reaction_n = 70, 
             delta_s = -0.414, model_path = 'PdO', calculator_method = 'lasp') 
    ...
    write_arc([best_atoms],  file_name = 'best_1.arc')