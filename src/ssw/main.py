...
def lasp_ssw_calc(atom, type):
    ...
    atom = read_arc('best.arc', index = -1)
    return atom, energy, force

def postprocessing(env:MCTEnv):
    atoms = read_arc('allstr.arc', index = 0)
    atoms = env.del_env_adsorbates(atoms)
    atoms, energy,_ = lasp_ssw_calc(atoms, 'l')
    return atoms, energy
...
if __name__ == '__main__':
    ...
    best_atoms, best_energy = postprocessing(env)
