...
J_TO_EV = 1 / (1.602 * 6.022 * 1E4)
KJ_TO_EV = 1E3 / (1.602 * 6.022 * 1E4)
K = 8.617 * 1E-5
...
class ASOP():
    ...
    def __post_init__(self) -> None:
        ...
        # △S_O2(473.15,p0) = 207.350 + (208.524 - 207.350) * 23.15 / 50 = 207.894 J/(mol * K) from NIST
        # H(Ag2O) = -31.1 KJ/mol, △fHm(Ag2O) = -0.323 eV        from CRC97
        # H(Ag2O) - 2H(Ag) = -0.314 eV                          from Material Project

        # µO2(T,p0) = (H(Ag2O) - 2H(Ag) - △fHm(Ag2O)) x 2 + △µO2(T,p0)
        # µO(T,p) = (µO2(T,p0=1bar) + kBTln(PO2/p0)) / 2 
        # △µO2(T,p0) = -T * S_O2 = -473.15 * 207.894 / 96485 = -1.02 eV
        
        Ag_bulk = read('Ag.poscar')
        Ag2O = read('Ag2O.poscar')
        H_Ag2O = self.calculator(Ag2O, calc_type = 'single') / int(len(Ag2O) / 3)
        H_Ag = self.calculator(Ag_bulk, calc_type = 'single') / len(Ag_bulk)
        # print(f"H_Ag2O = {H_Ag2O}, H_Ag = {H_Ag}")
        Hfm_Ag2O = -0.323   # eV
        self.E_O = ((H_Ag2O - 2 * H_Ag - Hfm_Ag2O) * 2 + -1.02 + K * 473.15 * np.log(self.P / 101325))/ 2
        # μAg = E_Ag_bulk / n_Ag
    ...
    def run(self) -> None:
        ...
        history = {}
        results = ['Ag', 'O', 'energy', 'miu', 'TM', 'grids', 'A']
        if os.path.exists('asop.npz'):
            data = np.load('asop.npz', allow_pickle = True)
            for item in results:
                history['miu'] = data['miu'].tolist()
                history['energy'] = data['energy'].tolist()
                history['Ag'] = data['Ag'].tolist()
                history['O'] = data['O'].tolist()
                history['TM'] = data['A_matrix'].tolist()
                history['grids'] = data['grids'].tolist()
                history['A'] = data['A'].tolist()
        else:
            for item in results:
                history[item] = []
        ...
        print(f"The total A is {possible_A}")
        ...
        for grid_index in range(len(possible_grids)):
            ...
            for n_Ag in range(A):
                ...
                for n_O in range(A): 
                    ...
                    # if os.path.exists(f'./save_dir/{norm_u}x{norm_v}/Ag{n_Ag}O{n_O}/best.arc'):
                    #     print(f"./save_dir/{norm_u}x{norm_v}/Ag{n_Ag}O{n_O}/best.arc exists")
                    #     continue
                    ...
                    try:
                        ...
                        arc_dir = f'./save_dir/{norm_u}x{norm_v}/Ag{n_Ag}O{n_O}'
                        if not os.path.exists(arc_dir):
                            os.mkdir(arc_dir)

                        new_state.pbc = True
                        write_dmol_arc(f'{arc_dir}/outstr.arc', [new_state])
                        shutil.copy('./best.arc', f'{arc_dir}/best.arc')
                        shutil.copy('./allstr.arc', f'{arc_dir}/allstr.arc')
                        shutil.copy('./allfor.arc', f'{arc_dir}/allfor.arc')
                        shutil.copy('./all.arc', f'{arc_dir}/all.arc')
                        ...
                        history['energy'].append(current_energy - initial_energy)
...
if __name__ == '__main__':
    initial_slab = fcc100('Ag', size=(20, 20, 4), vacuum=10.0)
